...
**기술 선택:** IntersectionObserver + CSS transition. scroll 이벤트 리스너는 스크롤마다 메인 스레드를 깨워 성능을 깎지만, IntersectionObserver는 브라우저가 교차 판정을 대신 해줘 비용이 없다. 라이브러리 불필요.

## 사용 방법

1. `assets/useScrollReveal.ts`·`assets/ScrollReveal.tsx`·`assets/scroll-reveal.css`를 프로젝트로 복사한다(전체 구현이 이 세 파일 — 반드시 읽어라).
...
const Features = ({ items }: { items: string[] }) => (
  <section>
    {items.map((text, i) => (
      <ScrollReveal key={text} delayMs={i * 100}>
