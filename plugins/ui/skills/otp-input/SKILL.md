---
name: otp-input
description: 인증번호(OTP) 칸 입력을 구현한다 — 한 글자를 치면 다음 칸으로 넘어가고, 빈 칸에서 지우면 앞 칸으로 돌아가며, 복사한 코드를 붙여넣으면 칸마다 하나씩 나뉜다. "인증번호 입력, OTP input, 6자리 코드 입력, 인증번호 칸, 본인확인 코드, 문자로 온 코드 붙여넣기" 요청과 SMS·이메일 인증 화면에 쓴다. 칸 수·에러 흔들림 수정에도 쓴다.
---
