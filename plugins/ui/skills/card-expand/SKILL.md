---
name: card-expand
description: 카드가 제자리에서 자라나 상세 화면이 되는 공유 요소 전환(App Store 투데이 카드식). 누른 카드의 그림·제목이 상세로 이어져 움직이고, 닫으면 원래 자리로 줄어든다. View Transitions API 기반. "카드 클릭하면 확대, 카드가 커지면서 상세로, 앱스토어 카드 전환, 공유 요소 전환, shared element, 히어로 전환, 목록에서 상세로 자연스럽게" 요청 시 반드시 이 스킬을 사용할 것. 기존 적용분의 속도·모서리·부위 수정 요청도 포함.
---
