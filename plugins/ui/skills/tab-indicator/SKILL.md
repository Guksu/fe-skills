---
name: tab-indicator
description: 탭을 바꿀 때 활성 탭 밑줄(tab indicator)이 옆으로 미끄러져 따라가는 애니메이션을 구현한다. "탭 밑줄 애니메이션, 탭 인디케이터, 슬라이딩 탭, 탭 누르면 밑줄이 옆으로 따라가는, 탭 바 밑줄" 요청과 상단 탭 바·카테고리 탭 UI에 쓴다. 기존 적용분의 속도·색·밑줄 두께 수정에도 쓴다.
---
