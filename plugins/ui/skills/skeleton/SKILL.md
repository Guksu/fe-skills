---
name: skeleton
description: 로딩 중 콘텐츠 자리를 잡아주는 스켈레톤 UI(skeleton loader)와 시머(반짝임) 애니메이션을 구현한다. "스켈레톤, 로딩 뼈대, 플레이스홀더, 시머 효과, 회색 박스가 반짝거리는 로딩, 데이터 불러오는 동안 자리 표시" 요청과 fetch 대기 UI에 쓴다. 기존 적용분의 반짝임 속도·색·형태 수정에도 쓴다.
---
