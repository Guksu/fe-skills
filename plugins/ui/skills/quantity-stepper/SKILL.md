...
description: 수량 조절 스테퍼(quantity stepper, − / + 버튼) 컴포넌트를 구현한다 — 꾹 누르고 있으면 점점 빨리 올라가고, 숫자를 직접 입력할 수 있으며, 1에서 −를 누르면 삭제로 이어진다. "수량 늘리고 줄이는 버튼, 플러스 마이너스로 개수 바꾸기, 장바구니 수량, 수량 버튼" 요청에 쓴다. 기존 적용분의 최대·최소 제한과 길게 누를 때 속도 수정에도 쓴다.
---
