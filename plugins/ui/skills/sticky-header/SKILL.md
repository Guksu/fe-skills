---
name: sticky-header
description: 스크롤하면 큰 제목이 밀려 나가고 상단 고정 헤더에 작은 컴팩트 제목이 나타나는 스티키 헤더(sticky header)를 구현한다. "스크롤하면 헤더 축소, 헤더 작아지게, 접히는/줄어드는 헤더, 내리면 큰 제목 사라지고 상단 바에 작은 제목, 고정 타이틀 전환" 요청과 상세 페이지·목록 상단에 쓴다. 기존 적용분의 축소 속도·배경 수정에도 쓴다.
---
