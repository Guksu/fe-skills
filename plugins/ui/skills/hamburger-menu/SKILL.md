---
name: hamburger-menu
description: 햄버거 버튼(≡, 삼선 아이콘)이 X로 모핑하며 옆에서 사이드 드로어가 밀려 나오는 모바일 내비게이션(hamburger menu)을 구현한다. "햄버거 메뉴, 사이드 메뉴, 드로어, 슬라이드 메뉴, 삼선 누르면 옆에서 메뉴 나오게, 모바일 전체 메뉴" 요청과 모바일 헤더의 내비게이션 패널에 쓴다. 기존 적용분의 방향(왼쪽/오른쪽)·속도·폭 수정에도 쓴다.
---
