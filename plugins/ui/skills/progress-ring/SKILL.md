---
name: progress-ring
description: 애플워치 활동 링처럼 원이 채워지는 진행 표시(프로그레스 링) 구현 — 값이 바뀌면 부드럽게 따라가고, 3중 링으로 겹쳐 쓸 수 있으며 값이 없으면 무한 로딩으로 돈다. "원형 진행률, 프로그레스 링, 도넛 진행 표시, 활동 링, 목표 달성률 원, 원형 로딩 스피너" 요청 시, 목표 달성·업로드·준비 진행률 UI를 만들 때 반드시 이 스킬을 사용할 것. 기존 적용분의 색·두께·속도 수정 요청도 포함.
---
