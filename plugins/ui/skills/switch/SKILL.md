---
name: switch
description: iOS 스타일 토글 스위치(toggle switch)를 구현한다 — 썸이 미끄러지고 트랙 색이 바뀌는 온오프 입력, 네이티브 input 기반이라 키보드·폼 연동이 된다. "스위치, 온오프 버튼, 켜기 끄기 토글, 알림 켜고 끄기, 설정 스위치" 요청과 설정·옵션의 즉시 적용형 on/off 입력에 쓴다. 기존 적용분의 크기·색·속도 수정에도 쓴다.
---
