---
name: range-slider
description: 두 손잡이로 최소·최대 구간을 고르는 범위 슬라이더(range slider, 가격·기간 필터) 컴포넌트를 구현한다 — 손잡이가 서로를 지나치지 않고 고른 구간이 색으로 채워지며 방향키로도 조절된다. "가격 범위 슬라이더, 최소값 최대값 손잡이 두 개, 구간 필터, 두 개짜리 슬라이더" 요청에 쓴다. 기존 적용분의 손잡이·채움 색·단계 수정에도 쓴다.
---
