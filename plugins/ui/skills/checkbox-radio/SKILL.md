---
name: checkbox-radio
description: 체크박스(checkbox)·라디오(radio) 버튼 컴포넌트 구현 — 체크할 때 체크 표시가 획으로 그려지듯 나타나고 라디오 도트가 튀어 맺히며, 키보드·그룹 배타·폼 연동이 된다. "체크박스, 라디오 버튼, 동의 체크, 약관 동의, 단일 선택, 다중 선택" 요청과 폼의 선택 입력에 쓴다. 기존 적용분의 크기·색·속도 수정에도 쓴다.
---
