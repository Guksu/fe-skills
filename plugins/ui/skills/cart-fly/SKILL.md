---
name: cart-fly
description: 담기 버튼을 누르면 상품 이미지가 장바구니 아이콘으로 포물선을 그리며 날아가 들어가는 애니메이션(add to cart fly)을 구현한다. "장바구니 담기 애니메이션, 카트로 날아가는 효과, 담을 때 슝 가는 효과, 아이템 플라이" 요청과 커머스 담기·찜 인터랙션에 쓴다. 기존 적용분의 속도·궤적 수정에도 쓴다.
---
