---
name: press-feedback
description: 버튼·카드를 탭하면 살짝 쪼그라들었다(움츠러들었다) 통 하고 튕겨 돌아오는 프레스 피드백(button press 눌림 효과)을 구현한다. "버튼 눌림 효과, 버튼 눌림 애니메이션, 누를 때 반응, 탭 피드백, 클릭 감각, 눌리는 느낌" 요청과 버튼·카드형 터치 타깃에 누름 반응을 넣을 때 쓴다. 기존 적용분의 줄어드는 배율·복귀 속도 수정에도 쓴다.
---
