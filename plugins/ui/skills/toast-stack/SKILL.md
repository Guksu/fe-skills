---
name: toast-stack
description: 화면 하단(또는 상단 배너 위치)에 토스트 알림이 쌓이고(스택) 시간이 지나면 스스로 사라지는 시스템 구현. iOS 푸시 배너처럼 위에서 내려오는 상단 모드 포함. "토스트 알림, 스낵바, 알림 쌓기, 저장됨 메시지 띄우기, 상단 배너 알림" 요청 시, 일시적 알림 UI를 만들 때 반드시 이 스킬을 사용할 것. 기존 적용분의 시간·개수·위치 수정 요청도 포함.
---
