---
name: story-progress
description: 스토리 화면 상단의 구간별 진행 막대(story progress)를 구현한다 — 자동 재생으로 칸이 차오르면 다음 장면으로 넘어가고, 길게 누르면 멈추고, 탭으로 이전·다음 이동. "스토리 진행바, 인스타 스토리 상단 바, 자동으로 넘어가는 온보딩, 쇼츠 진행 표시" 요청과 스토리·온보딩 화면에 쓴다. 기존 적용분의 구간 시간·구간 수 수정에도 쓴다.
---
