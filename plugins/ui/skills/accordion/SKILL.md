---
name: accordion
description: 아코디언(accordion) 컴포넌트 — 눌러서 펼치고 접는 목록을 구현한다. 질문을 누르면 답이 아래로 펼쳐지고 화살표가 돌며, 하나만 열리거나 여러 개 열리게 할 수 있다. "아코디언, FAQ 접기 펼치기, 펼쳐지는 목록, 접이식 섹션, 토글 섹션" 요청과 질문 목록·상세 스펙·설정 그룹 UI에 쓴다. 기존 적용분의 속도·단일/다중 열림 수정에도 쓴다.
---
